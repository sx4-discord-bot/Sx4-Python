...
class Economy:
    ...
    async def votebonus(self, ctx):
        ...
        if has_voted(ctx.message.author.id):
            ...
            await self.bot.say("You need to upvote the bot to use this command you can do that here: https://discordbots.org/bot/325196085159002112")
    ...
    async def russianroulette(self, ctx, bullets: int, bet: int):
        ...
        if bullets == 3:	
            if rr == 1 or rr == 2 or rr == 3:
                ...
                await self.bot.say(embed=s)
        if bullets == 4:	
            if rr == 1 or rr == 2 or rr == 3 or rr == 4:
                s=discord.Embed(description="You were shot :gun:\nYou lost your bet of **${}**".format(bet), colour=discord.Colour(value=colour))
                s.set_author(name=author.name, icon_url=author.avatar_url)
                await self.bot.say(embed=s)
            else:
                ...
                await self.bot.say(embed=s)
        if bullets == 5:	
            if rr == 1 or rr == 2 or rr == 3 or rr == 4 or rr == 5:
                s=discord.Embed(description="You were shot :gun:\nYou lost your bet of **${}**".format(bet), colour=discord.Colour(value=colour))
                s.set_author(name=author.name, icon_url=author.avatar_url)
                await self.bot.say(embed=s)
            else:
    ...
    async def bank(self, ctx, page: int=None):
        ...
        sortedbank = sorted(self.settings["user"].items(), key=lambda x: x[1]["balance"], reverse=True)[page*10-10:page*10]
    ...
    async def networth(self, ctx, page: int=None):
        ...
        networth_sorted = sorted(entries, key=lambda x: x["worth"], reverse=True)
        ...
        if index != -1:
            embed.set_footer(text="{}'s Rank: #{} | Page {}/{}".format(ctx.message.author.name, index + 1, page, math.ceil(len(self.settings["user"])/10)), icon_url = ctx.message.author.avatar_url)
        else:
            embed.set_footer(text = "{} does not have a rank | Page {}/{}".format(ctx.message.author.name, page, math.ceil(len(self.settings["user"])/10)), icon_url = ctx.message.author.avatar_url)
...
token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjMyNTE5NjA4NTE1OTAwMjExMiIsImJvdCI6dHJ1ZSwiaWF0IjoxNTIyNzAzOTY3fQ.O81cNpMDqcyvC1ck6Czj0ffXyCxPGKKwxA9nUFQBDe0"
