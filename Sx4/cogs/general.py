...
class General:
    ...
    async def google(self, ctx, *, search): 
        ...
        url = "https://www.googleapis.com/customsearch/v1?key=anapikeywouldgoherebutnotouchy".format(urllib.parse.urlencode({"q": search}))
        request = Request(url)
    ...
    async def dictionary(self, ctx, *, word): 
        ...
        request.add_header("app_id", "notouchy")
        request.add_header("app_key", "anapikeywouldgoherebutnotouchy")
        try:
            data = json.loads(urlopen(request).read().decode())
        except:
            await self.bot.say("No results :no_entry:")
    ...
    async def steam(self, ctx, *, profile_url: str=None):
        ...
        idurl = "http://api.steampowered.com/ISteamUser/ResolveVanityURL/v0001/?key=anapikeywouldgoherebutnotouchy&{}".format(urllib.parse.urlencode({"vanityurl": profile_url.replace("https://steamcommunity.com/id/", "")}))
        ...
        try:
            ...
            await self.bot.say("No results :no_entry:")
            return
        url = "http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=anapikeywouldgoherebutnotouchy&steamids={}".format(id)
        request = Request(url)
