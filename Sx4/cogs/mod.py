    ...
    async def on_message(self, message):
        if self.bot.is_logged_in:
            if self.initialized is False:
                ...
                for serverid in self.d:
                    server = self.bot.get_server(serverid)
                    role = discord.utils.get(server.roles, name="Muted - Sx4")
                    for userid in self.d[server.id]:
                        user = server.get_member(userid)      
                        time = self.d[server.id][user.id]["time"] - message.timestamp.timestamp() + self.d[server.id][user.id]["amount"] 
                        if self.d[server.id][user.id]["toggle"] == True:
                            if time <= 0:
                                await self.bot.remove_roles(user, role)
                                self.d[server.id][user.id]["time"] = None
                                self.d[server.id][user.id]["toggle"] = False
